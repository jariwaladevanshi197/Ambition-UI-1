    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-xs mt-0.5" style={{ color:"rgba(255,255,255,0.3)" }}>{products.length} products · {products.filter(p=>p.featured).length} featured</p>
        </div>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="grid gap-x-4 gap-y-1 mt-2" style={{ gridTemplateColumns:"repeat(3,auto) 1fr" }}>
                    {[
                      ...
                      <div key={s.label}>
                        <div className="text-[9px] font-bold mb-0.5" style={{ color:"rgba(255,255,255,0.25)", letterSpacing:"0.06em" }}>{s.label}</div>
                        <div className="text-[11px] font-semibold" style={{ color:"rgba(255,255,255,0.6)" }}>{s.val||"—"}</div>
                      </div>
                    ))}
                  </div>
                  <p className="text-[10px] mt-2" style={{ color:"rgba(255,255,255,0.3)" }}>Applications: {p.applications}</p>
                </div>
              </div>
              <div className="flex gap-2 shrink-0">
                <button onClick={()=>update({...p,featured:!p.featured})} className="p-2 rounded-lg transition-colors"
                        style={{ background:p.featured?"rgba(249,115,22,0.15)":"rgba(255,255,255,0.05)" }}>
                  <Star size={13} fill={p.featured?"var(--orange)":"transparent"} style={{ color:p.featured?"var(--orange)":"rgba(255,255,255,0.3)" }}/>
                </button>
