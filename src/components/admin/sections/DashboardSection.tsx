  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="text-xs mt-0.5" style={{ color:"rgba(255,255,255,0.3)" }}>Overview of your website content</p>
      </div>
        ...
        {stats.map(s=>(
          <div key={s.label} className="p-4 rounded-2xl transition-all"
            ...
            <div className="flex items-center justify-between mb-3">
              ...
              <TrendingUp size={12} style={{ color:"rgba(255,255,255,0.15)" }}/>
            </div>
            <div className="text-2xl font-black text-white mb-0.5">{s.value}</div>
            <div className="text-[10px] font-bold mb-0.5" style={{ color:"rgba(255,255,255,0.5)" }}>{s.label}</div>
            <div className="text-[9px]" style={{ color:"rgba(255,255,255,0.25)" }}>{s.sub}</div>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="rounded-2xl p-5" style={{ background:"rgba(255,255,255,0.03)", border:"1px solid rgba(255,255,255,0.07)" }}>
          <div className="flex items-center justify-between mb-4">
            <span className="text-sm font-bold text-white">Recent Messages</span>
            {unreadCount > 0 && (
            ...
            {recent.map(m=>(
              <div key={m.id} className="flex items-start gap-3 p-3 rounded-xl transition-colors"
                ...
                <div className="w-1.5 h-1.5 rounded-full mt-1.5 shrink-0"
                     style={{ background:m.read?"rgba(255,255,255,0.2)":"var(--orange)" }}/>
                <div className="flex-1 min-w-0">
                  <div className="text-xs font-semibold truncate" style={{ color:m.read?"rgba(255,255,255,0.5)":"white" }}>{m.name}</div>
                  <div className="text-[10px] truncate mt-0.5" style={{ color:"rgba(255,255,255,0.25)" }}>{m.message}</div>
                </div>
                <span className="text-[9px] shrink-0" style={{ color:"rgba(255,255,255,0.2)" }}>{m.date}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="rounded-2xl p-5" style={{ background:"rgba(255,255,255,0.03)", border:"1px solid rgba(255,255,255,0.07)" }}>
          <div className="flex items-center justify-between mb-4">
            <span className="text-sm font-bold text-white">Recent Activities</span>
          </div>
          <div className="flex flex-col gap-2">
            {recentActivities.map(a=>(
              <div key={a.id} className="flex items-start gap-3 p-3 rounded-xl transition-colors"
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-xs font-semibold truncate text-white">{a.title}</div>
                  <div className="flex items-center gap-2 mt-0.5">
                    ...
                    <span className={`text-[9px] px-1.5 py-0.5 rounded-full ${a.published?"text-green-400":"text-gray-400"}`}
                          style={{ background:a.published?"rgba(34,197,94,0.1)":"rgba(255,255,255,0.05)" }}>
                      {a.published?"Live":"Draft"}
                    </span>
                  </div>
                </div>
                <span className="text-[9px] shrink-0" style={{ color:"rgba(255,255,255,0.2)" }}>{a.date}</span>
              </div>
            ))}
          </div>
        </div>
