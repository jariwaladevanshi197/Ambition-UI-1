...
function NavBtn({
  ...
  return (
    <button onClick={onClick}
            ...
            style={{ background:active?"rgba(249,115,22,0.15)":"transparent", color:active?"var(--orange)":"rgba(255,255,255,0.45)" }}
            onMouseEnter={e=>{ if(!active){(e.currentTarget as HTMLButtonElement).style.background="rgba(255,255,255,0.05)";(e.currentTarget as HTMLButtonElement).style.color="white";}}}
    ...
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ background:"#0d0d0d" }}>
        <div className="w-full max-w-sm mx-4">
          ...
          <div className="p-7 rounded-3xl" style={{ background:"rgba(255,255,255,0.04)", border:"1px solid rgba(255,255,255,0.1)" }}>
            <h1 className="text-lg font-black text-white mb-1">Admin Panel</h1>
            <p className="text-[11px] mb-6" style={{ color:"rgba(255,255,255,0.35)" }}>
              Sign in to manage your website content
            </p>
            <form onSubmit={tryLogin} className="flex flex-col gap-4">
              <div>
                <label className="text-[10px] font-bold tracking-widest block mb-1.5" style={{ color:"rgba(255,255,255,0.4)" }}>
                  PASSWORD
                </label>
                <input
                  ...
                  style={{ background:"rgba(255,255,255,0.06)", border:`1px solid ${pwdErr?"#ef4444":"rgba(255,255,255,0.1)"}`, color:"white" }}
                />
                {pwdErr && (
  ...
  return (
    <div className="flex h-screen overflow-hidden" style={{ background:"#0d0d0d" }}>
      ...
      <aside className="flex flex-col shrink-0 transition-all duration-300 overflow-hidden"
             style={{ width:sidebar?220:0, minWidth:sidebar?220:0, background:"#111", borderRight:"1px solid rgba(255,255,255,0.07)" }}>
        ...
        <nav className="flex-1 py-4 px-2.5 overflow-y-auto">
          ...
          {groups.map(g=>(
            <div key={g} className="mt-5">
              <div className="px-3 mb-2 text-[9px] font-black tracking-widest" style={{ color:"rgba(255,255,255,0.2)" }}>{g}</div>
        ...
        {/* Logout */}
        <div className="px-2.5 py-4 shrink-0" style={{ borderTop:"1px solid rgba(255,255,255,0.07)" }}>
          <button onClick={()=>setLoggedIn(false)}
                  ...
                  onMouseLeave={e=>{ const b=e.currentTarget; b.style.background=""; b.style.color="rgba(255,255,255,0.35)"; }}>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="flex items-center justify-between px-6 shrink-0"
                style={{ height:56, borderBottom:"1px solid rgba(255,255,255,0.07)", background:"rgba(17,17,17,0.95)", backdropFilter:"blur(10px)" }}>
          <div className="flex items-center gap-3">
            <button onClick={()=>setSidebar(p=>!p)}
              ...
              {sidebar ? <X size={14} className="text-white"/> : <Menu size={14} className="text-white"/>}
            </button>
